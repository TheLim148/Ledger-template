...
def test_deposit_increase_balance(ledger: Ledger):
    ...
    assert (
        ledger.get_account_transactions(1)[0].transaction_type
...
def test_transfer_preserve_balance(ledger: Ledger):
    ...
    assert (
        ledger.get_account_transactions(account1.id)[0].transaction_type
        == TransactionType.TRANSFER
    )
    assert (
        ledger.get_account_transactions(account2.id)[0].transaction_type
        == TransactionType.TRANSFER
    )

    assert (
        ledger.get_account_transactions(account1.id)[0].from_account_id == account1.id
    )
