...
class PostgresRepository(LedgerRepository):
    def __init__(
        ...
        schema_path = (
            Path(__file__).resolve().parent.parent / "sql" / "schema_postgres.sql"
        )
    ...
    def _row_to_transaction(self, row: tuple) -> Transaction:
        ...
        return Transaction(
            row[0], TransactionType(row[1]), row[2], row[3], row[4], row[5]
        )
    ...
    def create_account(self, owner, balance=0) -> Account:
        ...
        crs.execute(
            "insert into accounts(owner, balance) values(%s, %s) returning *",
            (owner, balance),
        )
        row = crs.fetchone()
